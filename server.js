...
function finaliseMp4(hlsDir, mp4File){ return new Promise((ok,fail)=>{
  ...
  // asetpts=PTS-STARTPTS relabels audio timestamps to start at t=0 (no resampling, no silence insertion)
...
const server = http.createServer(async(req,res)=>{
  ...
  if(req.method==='POST'&&p==='/api/export'){
    ...
    //   Instead: re-encode in one pass with asetpts=PTS-STARTPTS
    ...
    // Video filter chain — always end with setpts to reset video timestamps to 0
    const videoFilter = vf ? `${vf},setpts=PTS-STARTPTS` : 'setpts=PTS-STARTPTS';

    // Encode args:
    // -bf 0 disables B-frames — eliminates the B-frame presentation delay that
    //        causes video to appear late relative to audio in some players.
    //        Source HLS already uses Constrained Baseline (no B-frames), so
    //        re-introducing them in the export only adds problems.
    const encodeArgs = [
    ...
    // Audio filter:
    // asetpts=PTS-STARTPTS relabels audio timestamps to start at 0 — identical
    // to setpts but for audio. Crucially it does NOT resample or insert silence.
    //
    // The previous aresample=async=1:first_pts=0 was the cause of the delay:
    // when video seeks to a keyframe slightly before inPoint and audio seeks to
    // exactly inPoint, first_pts=0 makes the resampler INSERT SILENCE from 0
    // to where audio actually begins — that silence is the "audio starts late"
    // the user was experiencing.
    const audioFilter = 'asetpts=PTS-STARTPTS';
    ...
    (async()=>{
      try{
        ...
        if(isLiveHls){
          ...
          // ── Two-pass: copy-extract → re-encode ───────────────────────────
          // Pass 1: stream-copy clip from the finished MP4.
          //   -avoid_negative_ts make_zero resets both stream timestamps to 0
          //   in one atomic operation, preserving the original A/V relationship.
          //   No filters, no re-encode = no introduced delay.
          await spawnFfmpeg([
            '-y',
            '-ss', String(inPoint),
            '-i', inputFile,
          ...
          // Pass 2: re-encode from the clean temp clip.
          //   Both streams already start at 0 from pass 1, so setpts/asetpts
          //   are just safety nets here — they won't shift anything.
          await spawnFfmpeg([
            '-y',
            '-i', tmpFile,
            '-map','0:v:0','-map','0:a:0',
